...
```{r}
d3
summary(lm(newsworthiness ~ condition * willsmith_perc, d3[topic == "willsmith"]))
summary(lm(newsworthiness ~ willsmith_perc, d3[condition == 0.5 & topic == "willsmith"]))
summary(lm(newsworthiness ~ willsmith_perc, d3[topic == "willsmith"]))
```
...
d0_feed_ws[, engage := likes_1 + shares_1 + read_later_1]

coefs <- d0_feed_ws[condition == 0.5, .(b1 = coef(lm(ldwell ~ item_order))[2]), user_id]
coefs

feed_avg <- d0_feed_ws[, .(like = mean(likes_1, na.rm = T),
...
d5 <- left_join(d3, feed_avg) |> left_join(coefs) |> data.table()
d5

library(hausekeep)
summaryh(lm(newsworthiness ~ like, d5[topic == "willsmith" & condition == 0.5]))
summaryh(lm(newsworthiness ~ read, d5[topic == "willsmith" & condition == 0.5]))
summaryh(lm(newsworthiness ~ share, d5[topic == "willsmith" & condition == 0.5])) # 
...
summaryh(lm(rating_combined ~ like + read + ldwell, d5[topic == "willsmith" & condition == 0.5]))
...
ggplot(d5[condition == 0.5], aes(like, importance)) +
    geom_jitter() +
    geom_smooth(method = 'lm', alpha = 0.1) 


library(fixest)
feols(ldwell ~ btn_likes | user_id, d0_feed_ws[engage == 0])
feols(ldwell ~ btn_shares | user_id, d0_feed_ws[engage == 0])

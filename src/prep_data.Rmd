...
d1[, unique(response)]
d1[, news_eval := as.numeric(substr(response, 1, 1))]
glimpse(d1)
```
...
# extra quick analyses
d2[, .N, keyby = .(user_id, condition, engagement_type)]
d2[, .N, keyby = .(user_id, condition, engagement_type)][, .(mean(N)), keyby = .(condition, engagement_type)]
d2[, .N, user_id]
...
```{r}
users_wrongrows <- d4[, .N, user_id][N != 6, user_id]
users_wrongrows

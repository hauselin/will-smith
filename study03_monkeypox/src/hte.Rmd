...
```{r}
y <- d1$y
df_X <- select(d1, -user_id, -y, -condition)
names(df_X)
X <- as.matrix(df_X)
w <- d1$condition
w  
...
```{r}
cf <- causal_forest(X, y, w, num.trees = 4000, mtry = ncol(X))
cf
...
```{r}
plot(get_tree(cf, 1))  # plot one example treea
```

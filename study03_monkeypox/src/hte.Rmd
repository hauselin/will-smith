...
# outcome variable
y <- "monkeypox_importance"  
d0 <- d00[field == y, .(user_id, field, y = news_eval, condition)]
d0[, y := as.numeric(y)]
...
n <- 50
vv <- "age"
pred_vals <- seq(0, 90, length.out = n)
...
ggplot(cf_predict, aes(v, predictions)) +
    geom_point() +
    geom_errorbar(aes(ymin = predictions - 1.96 * se, ymax = predictions + 1.96 * se), width = 0) +
    labs(x = vv, y = "average treatment effect")
```

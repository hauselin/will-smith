...
item_avg <- d4[, .(value = mean(value, na.rm = T)), keyby = .(question, topic_id, topic)]
item_avg_wide <- dcast(item_avg, topic_id + topic ~ question, value.var = "value")
item_avg_wide <- select(item_avg_wide, topic_id, topic, importance, newsworthy, familiarity, desiredFrequency, observedFrequency) |> 
    arrange(importance)
